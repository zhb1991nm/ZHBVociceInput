...
- (UIView *)viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image action:(SEL)action;
...
- (void)makeToast:(NSString *)message duration:(CGFloat)interval position:(id)position {
    UIView *toast = [self viewForMessage:message title:nil image:nil action:nil];
    [self showToast:toast duration:interval position:position];  
}

- (void)makeToast:(NSString *)message duration:(CGFloat)interval position:(id)position action:(SEL)action{
    UIView *toast = [self viewForMessage:message title:nil image:nil action:action];
    [self showToast:toast duration:interval position:position];
}

- (void)makeToast:(NSString *)message duration:(CGFloat)interval position:(id)position title:(NSString *)title {
    UIView *toast = [self viewForMessage:message title:title image:nil action:nil];
    [self showToast:toast duration:interval position:position];  
}

- (void)makeToast:(NSString *)message duration:(CGFloat)interval position:(id)position image:(UIImage *)image {
    UIView *toast = [self viewForMessage:message title:nil image:image action:nil];
    [self showToast:toast duration:interval position:position];  
}

- (void)makeToast:(NSString *)message duration:(CGFloat)interval  position:(id)position title:(NSString *)title image:(UIImage *)image {
    UIView *toast = [self viewForMessage:message title:title image:image action:nil];
    [self showToast:toast duration:interval position:position];  
}
...
- (void)makeToastActivityWithText:(NSString *)text{
    id position = CSToastActivityDefaultPosition;
    ...
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, activityView.bounds.size.height / 2 + 20, activityView.bounds.size.width, 20)];
    ...
    if (text) {
        ...
        label.text = NSLocalizedString(@"Toast_Strong_00", @"处理中..");
    }
...
//        button.frame = CGRectMake(0.0, 0.0, 100 , 50);
//        [button setBackgroundColor:[UIColor clearColor]];
//        [button addTarget:self action:action forControlEvents:UIControlEventTouchUpInside];
//        [button setTitle:@"点击查看" forState:UIControlStateNormal];
//        button.top = messageLabel.bottom + CSToastVerticalPadding;
//        buttonHeight = 50 + CSToastVerticalPadding;
